        ...
        public async Task Run(CancellationToken cancellation)
        {
            ...
            var m = await await
                    Task.WhenAny(
                        ...
                        Task.Delay(TimeSpan.FromSeconds(10)).ContinueWith(t => (ApiCommandWithName) null));
            if (m == null)
            {
            ...
            while (!cancellation.IsCancellationRequested)
            {
                ...
                if (m.Text == "CalculateAndRespond")
                {
                    await _serializer.SerializeAsync(
                        new ApiCommandWithName { Text = string.Format("Calculation result from {0} : {1}", ClientName, _counter++) },
